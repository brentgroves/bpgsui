...
class ChangePassword extends Component {
  ...
  handleSubmit = async event => {
    ...
    await login(this.state.email, this.state.oldPassword)
      .then(async function (user) {
        await thisLv1.changePassword(user, thisLv1.state.email, thisLv1.state.oldPassword, thisLv1.state.newPassword)
  ...
  renderForm() {
    const { emailStatus, oldPasswordStatus, newPasswordStatus, loading, formStatus } = this.state
